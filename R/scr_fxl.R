...
#' @return
#' @export
scr_arrows <- function(core_frame, arrows = NULL, facet = NULL,
...
#' @return
#' @export
scr_brackets <- function(core_frame, brackets = NULL, facet = NULL,
...
#' @return
#' @export
scr_bar_support <- function(core_frame, color = rgb(.8, .8, .8, alpha = 0.25),
...
#' @return
#' @export
scr_cumsum_lines <- function(core_frame, lty = 1, color = "black",
                       size = 1, mapping) {
...
#' @return
#' @export
scr_cumsum_points <- function(core_frame, pch = 21, color = "black",
...
#' @return
#' @export
scr_lines <- function(core_frame, lty = 1, color = "black",
                      size = 1, mapping) {
...
#' @return
#' @export
scr_guide_line <- function(core_frame, coords, facet = NA,
...
#' @return
#' @export
scr_plines <- function(core_frame, lines = NULL,
...
#' @return
#' @export
scr_plines_mbd <- function(core_frame, lines = NULL) {
...
#' @return
#' @export
scr_points <- function(core_frame, pch = 21, color = "black",
                       fill = "black", cex = 1, mapping) {
...
#' @return
#' @export
scr_xlabel <- function(core_frame, var, line = 0) {
...
#' @return
#' @export
scr_xoverride <- function(core_frame, var, xdelta = 1,
...
#' @return
#' @export
scr_ylabel <- function(core_frame, var, line = 0) {
...
#' @return
#' @export
scr_yoverride <- function(core_frame, var, ydelta = 1, ydraws = NULL) {
...
#' @return
#' @export
scr_title <- function(core_frame, var) {
...
#' @return
#' @export
scr_legend <- function(core_frame, panel = NA, legend,
  ...
  core_frame$legendpars[["pt.bg"]] = pt.bg
  ...
  core_frame$legendpars[["box.lty"]] = box.lty
  core_frame$legendpars[["cex"]] = cex
  core_frame$legendpars[["horiz"]] = horiz
  core_frame$legendpars[["position"]] = position
  core_frame$legendpars[["pt.cex"]] = pt.cex
  core_frame$legendpars[["text.col"]] = text.col
...
print.fxlsemilog <- function(core_frame, ...) {
  core_frame$dims[["min.local.x"]]   = min(core_frame$data[[
    as.character(core_frame$aes["x"])]], na.rm = TRUE)
  core_frame$dims[["max.local.x"]]   = max(core_frame$data[[
    as.character(core_frame$aes["x"])]], na.rm = TRUE)
  ...
  labelLogicals <- c(TRUE, FALSE, FALSE, TRUE, FALSE, FALSE, FALSE, FALSE, TRUE)
  ...
  axis(2,
       at     = c(0.1,
                  as.vector(c(1) %o% 10^(log10(core_frame$dims[["min.local.y"]]):log10(core_frame$dims[["max.local.y"]])))),
       las    = 1,
       tcl    = par("tcl"),
       labels = c(0.1,
                  as.vector(c(1) %o% 10^(log10(core_frame$dims[["min.local.y"]]):log10(core_frame$dims[["max.local.y"]])))))
  ...
  abline(h = c(0.1,
               as.vector(c(5) %o% 10^(log10(core_frame$dims[["min.local.y"]]):log10(core_frame$dims[["max.local.y"]])))),
         lty = 3,
         col = "darkblue")
  ...
  if (length(core_frame[["layers"]]) > 0) {
    for (i in 1:length(core_frame[["layers"]])) {

      current_layer = core_frame$layers[[i]]
      current_layer$facet = "hack"

      if (current_layer$type == "arrows")         draw_arrows(core_frame, current_layer, "hack")
      if (current_layer$type == "brackets")       draw_brackets(core_frame, current_layer, "hack")
      if (current_layer$type == "guide_line")     draw_guide_line(core_frame, current_layer, "hack")
      if (current_layer$type == "line")           draw_lines(core_frame, current_layer, NA)
      if (current_layer$type == "phase_label")    draw_label_phase(core_frame, current_layer, "hack")
      if (current_layer$type == "point")          draw_points(core_frame, current_layer, NA)
    }
  }
  ...
  if (length(core_frame[["layers"]]) > 0)
    for (i in 1:length(core_frame[["layers"]]))
      if (core_frame$layers[[i]]$type == "point")
        draw_points(core_frame, core_frame$layers[[i]], NA, zeroAxis = TRUE)

  mtext(core_frame$labs[["ylab"]],  side = 2, outer = TRUE)
  mtext(core_frame$labs[["xlab"]],  side = 1, outer = TRUE)
}
...
print.fxl <- function(core_frame, ...) {
  ...
  fontFamily = ifelse(is.null(core_frame[["family"]]), "serif", core_frame[["family"]])
  ...
  for (facetIndex in 1:n_facets) { # Print placeholders
    ...
    # X axes
    x.axis.draw  = (facetIndex == n_facets)

    if (!is.null(core_frame$dims[["xdraws"]])) {
      x.axis.draw =  current_facet %in% core_frame$dims[["xdraws"]]
    }
    ...
    if (!is.null(core_frame$dims[["xticks"]]) & is.list(core_frame$dims[["xticks"]])) {
      x.axis.ticks = core_frame$dims[["xticks"]][[ current_facet]]
    ...
    if (!is.null(core_frame$dims[["ydraws"]])) {
      y.axis.draw =  current_facet %in% core_frame$dims[["ydraws"]]
    }

    y.axis.ticks = seq(core_frame$dims[["global.min.y"]],
                       core_frame$dims[["global.max.y"]],
    ...
    if (!is.null(core_frame$dims[["local.dims"]])) {
      ...
      y.axis.ticks = seq(core_frame$dims[["min.local.y"]],
                         core_frame$dims[["max.local.y"]],
                         by = core_frame$dims[['ydelta']])

      if ("yticks" %in% names(core_frame$dims[["local.dims"]][[ current_facet]]))
        y.axis.ticks = core_frame$dims[["local.dims"]][[ current_facet]]$yticks
    ...
    if (!is.null(core_frame$dims[["xticklabs"]]) &
        ...
        x.axis.draw) {
      x.axis.draw = core_frame$dims[["xticklabs"]]
    }
      ...
      for (i in 1:length(core_frame[["layers"]])) {
        ...
        if (current_layer$type == "mbd_phase_lines") {
          ...
          for (pname in plines) {
            ...
            currentIndex = which(names(current_layer$lines[[pname]]) ==  current_facet)

            if (length(currentIndex) == 0) next

            tmpX1 = current_layer$lines[[pname]][[currentIndex]][["x1"]]
            tmpX2 = ifelse(is.null(current_layer$lines[[pname]][[currentIndex]][["x2"]]),
                           tmpX1,
                           current_layer$lines[[pname]][[currentIndex]][["x2"]])

            tmpY1 = ifelse(is.null(current_layer$lines[[pname]][[currentIndex]][["y1"]]),
                           0,
                           current_layer$lines[[pname]][[currentIndex]][["y1"]])
            tmpY2 = ifelse(is.null(current_layer$lines[[pname]][[currentIndex]][["y2"]]),
                           0,
                           current_layer$lines[[pname]][[currentIndex]][["y2"]])

            current_layer$lines[[pname]][[currentIndex]][["topDraw"]] <- cnvrt.coords(
              tmpX1,
              core_frame$dims[["max.local.y"]])

            current_layer$lines[[pname]][[currentIndex]][["botDraw"]] <- cnvrt.coords(
              tmpX2,
              -((core_frame$dims[["max.local.y"]] - core_frame$dims[["min.local.y"]]) * 0.04))

            tmp.point.top.dev <- cnvrt.coords(
              current_layer$lines[[pname]][[currentIndex]][["topDraw"]]$dev,
              input = 'dev')

            tmp.point.bot.dev <- cnvrt.coords(
              current_layer$lines[[pname]][[currentIndex]][["botDraw"]]$dev,
              input = 'dev')
  ...
  if (reqDraw) {
    ...
    for (pl in 1:length(n.phaseLines)) {
      ...
      n_facets <- length(plotTops[[n.phaseLines[pl]]])
